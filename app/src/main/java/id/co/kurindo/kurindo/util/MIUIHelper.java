...
public class MIUIHelper {
	...
	private MIUIHelper() {
		...
		if (mIsMIUI_V5) {
			LogUtil.logD(TAG, "this is MIUI 5");
		} else if (mIsMIUI_V6) {
			LogUtil.logD(TAG, "this is MIUI 6");
		}
...
//					"com.miui.permcenter.autostart.AutoStartManagementActivity"));
	...
	public boolean isFloatWindowOpAllowed(Context context) {
		...
		if (mIsMIUI_V5) {
			...
			LogUtil.logD(TAG, "flags:" + flags + ", flag:" + flag + ", flag2:"
					+ flag2 + ", op:" + op);
			if (flag > 0) {
				return true;
			} else {
	...
	private boolean checkOp(Context context, int op) {
		...
		if (version >= 19) {
			...
			LogUtil.logE(TAG, "Below API 19 cannot invoke!");
		}
		return false;
	}
	...
	private void getApplicationInfoFileds() {
		...
		for (Field field2 : field) {
			...
			LogUtil.logD(TAG, "fieldName:" + fieldName);
			if (fieldName.equalsIgnoreCase(FLAG_FLOATING_WINDOW)) {
				...
				LogUtil.logD(TAG, "Bit_Floating_Window:" + BIT_FLOATING_WINDOW);
				...
				LogUtil.logD(TAG, "Bit_Auto_Reboot:" + BIT_DISABLE_AUTOSTART);
			}
		}
	}
	...
	private void startActivity(Context context, Intent intent) {
		if (isIntentAvailable(context, intent)) {
			...
			LogUtil.logD(TAG, "intent is illegal");
		}
	}
